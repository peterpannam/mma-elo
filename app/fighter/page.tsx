...
export async function generateMetadata({
  ...
  if (a && b) {
    const [fighterA, fighterB] = await Promise.all([getFighterBySlug(a), getFighterBySlug(b)])
  ...
  return {
    title: 'Fighters',
    description: 'Search 5000+ UFC fighters and compare ELO career timelines.',
    openGraph: { title: 'Fighters — The ELO Almanac', description: 'Search 5000+ UFC fighters and compare ELO career timelines.' },
  }
}

function buildSeries(name: string, color: string, history: History, wc: string): ChartSeries | null {
  const points =
    wc === 'P4P'
      ? history
...
function calcStats(history: History, wc: string) {
  if (wc === 'P4P') {
    const rows = history.filter(e => e.p4p_elo_after != null).sort((a, b) => b.date.localeCompare(a.date))
    return {
  ...
  const rows = history.filter(e => e.weight_class === wc).sort((a, b) => b.date.localeCompare(a.date))
  return {
...
export default async function FighterIndexPage({
  ...
  const compareHeading = bothSelected ? `${fighterA.name} vs ${fighterB.name}` : 'Compare Fighters'

  const statRows = [
    { label: 'Current ELO', a: statsA?.current ?? null, b: statsB?.current ?? null },
    { label: 'Peak ELO',    a: statsA?.peak    ?? null, b: statsB?.peak    ?? null },
    { label: wc === 'P4P' ? 'Total fights' : `${wc} fights`, a: statsA?.fights ?? null, b: statsB?.fights ?? null },
  ]
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-10 items-start">
      ...
      <div>
        ...
        <p className="font-mono text-xs text-muted mt-1 mb-6">Search by name</p>
        <FighterSearch initialFighters={fighters} />
      </div>

      {/* Right: Compare */}
      <div style={{ borderLeft: '1px solid var(--color-rule)', paddingLeft: '2.5rem' }}>
        <Kicker>Fighter Comparison</Kicker>
        <SectionHeader>{compareHeading}</SectionHeader>
              ...
              {statRows.map(({ label, a: valA, b: valB }) => (
                <div key={label} className="contents">
                  ...
                  <div className="border-t border-l border-rule px-3 py-2.5 text-center">
                    <span className={`font-mono text-sm font-semibold ${valA != null && valB != null && valA > valB ? 'text-ink' : 'text-muted'}`}>
                      {valA ?? '—'}
                    </span>
                  </div>
                  <div className="border-t border-l border-rule px-3 py-2.5 text-center">
                    <span className={`font-mono text-sm font-semibold ${valA != null && valB != null && valB > valA ? 'text-ink' : 'text-muted'}`}>
                      {valB ?? '—'}
                    </span>
                  </div>
                </div>
